...
export function EditUserDialog({ open, name: initName, dailyWage: initWage, onSave, onClose }: Props) {
  ...
  useEffect(() => {
    if (open) {
      setName(initName)
      setWage(initWage ? String(initWage) : '')
    }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50" onClick={onClose}
      ...
      <form onSubmit={handleSubmit} onClick={(e) => e.stopPropagation()}
        className="w-[380px] p-6" style={{ background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-lg)' }}>
        ...
        <div className="mt-3">
          ...
          <p className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>
            Lương cơ bản của người này. Công trường có lương riêng sẽ ghi đè.
          </p>
        </div>
